...
use async_openai::types::{ChatCompletionMessageToolCallChunk, FunctionCall, FunctionCallStream};
...
use crate::llm::universal::{ChatCompletionError, ChatCompletionErrorResponse, MessageToolCall};
...
pub struct LLMResponse {
	...
	pub tool_calls: Option<Vec<universal::MessageToolCall>>,
...
impl AIProvider {
		...
		let (llm_resp, body) = match openai_response {
			Ok(mut success) => {
				let llm_resp = LLMResponse {
					...
					tool_calls: if include_completion_in_log {
						Some(
							success
								.choices
								.iter()
								.flat_map(|c| c.message.tool_calls.clone())
								.flatten()
								.collect_vec(),
						)
					} else {
	...
	async fn default_process_streaming(
		...
		let mut tool_calls = if include_completion_in_log {
			Some(Vec::new())
		} else {
			None
		};
			...
			parse::sse::json_passthrough::<universal::StreamResponse>(b, move |f| {
				match f {
					Some(Ok(f)) => {
						...
						if let Some(c) = tool_calls.as_mut()
							&& let Some(delta) = f.choices.first().and_then(|c| c.delta.tool_calls.clone())
						{
							for d in delta {
								if let ChatCompletionMessageToolCallChunk {
									id: Some(id),
									r#type: Some(t),
									function:
...
pub struct SimpleToolCall {
	...
	pub function: SimpleToolFunctionCall,
}

#[apply(schema!)]
pub struct SimpleToolFunctionCall {
...
impl From<&universal::MessageToolCall> for SimpleToolCall {
	fn from(msg: &universal::MessageToolCall) -> Self {
		SimpleToolCall {
			id: msg.id.clone(),
			function: SimpleToolFunctionCall {
				name: msg.function.name.clone(),
				arguments: msg.function.arguments.clone(),
			},
		}
	}
}
