...
impl Gateway {
	...
	pub(crate) async fn run_bind(
		...
		// Serve normally until the minimum drain time has elapsed after SIGTERM.
		// Then close the listener and gracefully drain existing HTTP connections, allowing
		// in-flight requests to finish. Exit when connections finish or the maximum time
		// since SIGTERM is reached, whichever comes first.
		let accept = |drain: DrainWatcher, force_shutdown: watch::Receiver<()>| async move {
			...
			// Disable the upgrader when accepting stops so only connections block drain completion.
			...
			upgrader.disable();
			drop(drain_mode);
			// Returning would force-close the spawned connections. Let run_with_drain
			// wait for their drain blockers or the maximum deadline instead.
