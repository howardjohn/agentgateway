...
#[tokio::test]
async fn drain_stops_accepting_after_minimum() {
	...
	// A connection accepted during the minimum must be tracked until its request finishes.
	let request = tokio::spawn(get(http1_client(), addr));
	in_flight.recv().await.unwrap();
	...
	.await
	.expect("listener must close while the request is still in flight");
	assert!(start.elapsed() >= Duration::from_secs(1));
	assert!(!drained.is_finished(), "must wait for the active request");

	let headers = assert_full_response(request.await.unwrap().unwrap()).await;
	assert_eq!(headers[header::CONNECTION], "close");
	tokio::time::timeout(Duration::from_secs(1), drained)
		.await
		.expect("drain must finish after the request completes")
		.unwrap();
}
