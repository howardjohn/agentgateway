...
func (s *BaseTestingSuite) SetupSuite() {
	...
	done := traceStep(s.T(), "detected Gateway API version")
	...
	done = traceStep(s.T(), "setup suite helpers")
	s.setupHelpers()
	done()
...
func (s *BaseTestingSuite) ApplyManifests(testCase *TestCase) {
	...
	if len(testCase.Manifests) > 0 {
		done = traceStep(s.T(), "applied manifests %v", manifestNames(testCase.Manifests))
	}
	...
	for manifest, transform := range testCase.ManifestsWithTransform {
		done := traceStep(s.T(), "applied transformed manifest %s", filepath.Base(manifest))
	...
	done = traceStep(s.T(), "manifest resources ready")
	...
	done = traceStep(s.T(), "dynamic resources ready")
	...
	for _, resource := range allResources {
		...
		done := traceStep(s.T(), "pods ready %s/%s", ns, name)
...
func (s *BaseTestingSuite) Send(target string, expect *testmatchers.HttpResponse, opts ...curl.Option) {
	s.T().Helper()
	BaseGateway.Send(s.T(), expect, append(targetOptions(s.T(), target), opts...)...)
}
